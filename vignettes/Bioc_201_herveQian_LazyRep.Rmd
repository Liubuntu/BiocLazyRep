...
#### CoreArray & Genomic Data Structure
...
The GDS file inside R is using hierarchical structure: 
...
<!-- `r knitr::include_graphics(system.file(package='BiocWorkshops2019', 'vignettes', 'GDS_hierarchy.jpg'))` -->

by supporting specific functions like: 
...
These functions corresponds to the GDS data structure and works best
 ...
 time, it also requires much efforts from users to be familiar with
 the whole set of strategies and functions in order to make full use
 of these packages.

#### `GDSArray`, `GDSMatrix`, and `GDSFile`

To fully make use of the GDS format, and at the same time provide
...
```
FIXME (auto completion): Try typing in `gf$ann` and pressing `tab` key

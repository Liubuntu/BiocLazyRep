...
### The DelayedDataFrame internals (for developers only)
...
### The SQLDataFrame internals (for developers only)
...
- In particular, DelayedArray backend authors should be able to
  implement optimized backend-specific methods (e.g. `max`),
  and the methods for DelayedArray objects should be able to take
  advantage of these optimized methods instead of triggering _block
  processing_.
...
- Explore other backends for on-disk DataFrame-like objects (Spark?)

- SQLDataFrame don't extend DataFrame at the moment:

    ```{r}
    showClass("SQLDataFrame")
    ```

    This prevents these objects (and other future DataFrame-like objects)
    from being used to store the metadata columns of a Vector derivative.

    Solution:
    - rename DataFrame (e.g. -> SimpleDataFrame)
    - re-introduce DataFrame as a _virtual_ class with no slots
    - make SimpleDataFrame, SQLDataFrame, and future DataFrame-like classes
      inherit from DataFrame (_concrete_ subclasses)

...
The only difference between `A0` and `A1` is that `A1` now carries _delayed
operations_.
...
### On-disk backend vs in-memory backend

TODO (by Hervé)
On-disk backend: e.g. HDF5Array objects
- `path(A0)`
- `h5ls(path(A0))`
...
- Don't miss the `Effectively using the DelayedArray framework to support
  ...
  want to learn about DelayedArray objects.
